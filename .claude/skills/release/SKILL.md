...
- **The gate comes after the PR is approved.** `main` requires one approving review, so the checklist asks you to get the PR reviewed and approved first, then say go. The skill checks the approval once and merges; it doesn't wait or poll for it.
...
3. >>> YOU: review the checklist (changelog, sha, soak confirmation), get the PR approved by another maintainer, then say go. <<<
...
main requires one approving review: get the PR approved before you say go. I'll check, not wait.
...
Then check the approval once — the user was asked to get it before saying go:
...
If it isn't approved, stop and say so. Don't poll; the user comes back when it is.
...
The main-moved guard runs twice on purpose: once when Step 2 starts and once right before the merge, because review time is exactly when someone else lands a commit on main.
