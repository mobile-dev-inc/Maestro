...
class TestCommand : Callable<Int> {
    ...
    override fun call(): Int {
        ...
        val result = try {
            handleSessions(debugOutputPath, executionPlan, resolvedTestOutputDir, driverPort?.toInt())
        } catch (e: Exception) {
...
//        }
    ...
    private suspend fun runMultipleFlows(
        ...
        // Read recording metadata from env map (no defaults - if not set, recording will be disabled)
