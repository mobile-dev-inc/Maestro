...
class DeviceCoreRoutingTest {
    ...
    // --- route(condition): assertVisible / assertNotVisible on a literal-text selector ---
    ...
    @Test fun `id selector does NOT route as an assert`() {
    ...
    // --- routeIdTap(selector): tapOn a literal-id selector ---

    @Test fun `plain id selector routes for tap`() {
        assertThat(DeviceCoreRouting.routeIdTap(ElementSelector(idRegex = "login_btn"))).isEqualTo("login_btn")
    }

    @Test fun `text selector does NOT route for tap`() {
        assertThat(DeviceCoreRouting.routeIdTap(ElementSelector(textRegex = "Login"))).isNull()
    }

    @Test fun `id selector with regex metacharacters does NOT route for tap`() {
        assertThat(DeviceCoreRouting.routeIdTap(ElementSelector(idRegex = "btn_.*"))).isNull()
    }

    @Test fun `id selector with an index does NOT route for tap`() {
        assertThat(DeviceCoreRouting.routeIdTap(ElementSelector(idRegex = "btn", index = "1"))).isNull()
    }

    @Test fun `id selector with a relative constraint does NOT route for tap`() {
        val sel = ElementSelector(idRegex = "btn", below = ElementSelector(textRegex = "Header"))
        assertThat(DeviceCoreRouting.routeIdTap(sel)).isNull()
    }
}
