...
data class YamlFluentCommand(
  ...
  private fun _toCommands(flowPath: Path, appId: String): List<MaestroCommand> {
    return when {
      ...
      inputRandomCityName != null -> listOf(
        MaestroCommand(
          InputRandomCommand(
            inputType = InputRandomType.TEXT_CITY_NAME,
            label = inputRandomCityName.label,
            optional = inputRandomCityName.optional
          )
        )
      )

      inputRandomCountryName != null -> listOf(
        MaestroCommand(
          InputRandomCommand(
            inputType = InputRandomType.TEXT_COUNTRY_NAME,
            label = inputRandomCountryName.label,
            optional = inputRandomCountryName.optional
          )
        )
      )

      inputRandomColorName != null -> listOf(
        MaestroCommand(
          InputRandomCommand(
            inputType = InputRandomType.TEXT_COLOR,
            label = inputRandomColorName.label,
            optional = inputRandomColorName.optional
          )
        )
      )
      ...
      setOrientation != null -> listOf(
        MaestroCommand(
          SetOrientationCommand(
            orientation = DeviceOrientation.getByName(setOrientation.orientation)
              ?: throw SyntaxError("Unknown orientation: $setOrientation"),
  ...
  private fun installAppCommand(installApp: YamlInstallApp, flowPath: Path): InstallAppCommand {
    ...
    val resolvedPath =
      ...
      else flowPath.resolveSibling(path)
        .toAbsolutePath()
        .normalize()
  ...
  private fun readConfig(flowPath: Path, commandFile: String): MaestroConfig? {
    ...
    return YamlCommandReader.readConfig(runFlowPath)
      .toCommand(runFlowPath).applyConfigurationCommand?.config
  }
