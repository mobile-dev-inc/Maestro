...
class Orchestra(
    ...
    private fun runScriptCommand(command: RunScriptCommand): Boolean {
        return if (evaluateCondition(command.condition, commandOptional = command.optional)) {
            ...
            val script = if (scriptPath != null && flowPath != null) {
                try {
                    val finalPath = resolvePathWithAliases(scriptPath, flowPath)
                    finalPath.readText()
                } catch (e: Exception) {
    ...
    private suspend fun runFlowCommand(command: RunFlowCommand, config: MaestroConfig?): Boolean {
        return if (evaluateCondition(command.condition, command.optional)) {
            ...
            val commands = if (flowFilePath != null && parentFlowPath != null) {
                try {
                    val finalPath = resolvePathWithAliases(flowFilePath, parentFlowPath)
    ...
    /**
     * Resolves a file path that may contain path aliases or JS interpolation.
     * First tries to resolve using WorkingDirectory aliases, then falls back to relative path resolution.
     */
    private fun resolvePathWithAliases(targetPath: String, parentPath: String): Path {
        val pathObj = java.nio.file.Paths.get(parentPath)
        val resolvedPath = if (WorkingDirectory.pathAliases.isNotEmpty()) {
            val resolved = WorkingDirectory.resolve(targetPath)
            if (resolved.isAbsolute) resolved.toPath() else null
        } else null
        ...
        return if (resolvedPath != null) {
            ...
            if (p.isAbsolute) p else pathObj.resolveSibling(p).toAbsolutePath()
        }
    }
