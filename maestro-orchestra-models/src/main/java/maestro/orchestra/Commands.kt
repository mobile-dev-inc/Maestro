...
data class ScrollUntilVisibleCommand(
  ...
  override fun evaluateScripts(jsEngine: JsEngine): ScrollUntilVisibleCommand {
    return copy(
      ...
      scrollDuration = scrollDuration.evaluateScripts(jsEngine)
        .speedToDuration(),
...
data class InputRandomCommand(
  ...
  fun genRandomString(): String {
    ...
    return when (inputType) {
      InputRandomType.NUMBER -> faker.number()
        .randomNumber(finalLength)
        .toString()

      InputRandomType.TEXT -> faker.text()
        .text(finalLength)

      InputRandomType.TEXT_EMAIL_ADDRESS -> faker.internet()
        .emailAddress()

      InputRandomType.TEXT_PERSON_NAME -> faker.name()
        .name()

      InputRandomType.TEXT_CITY_NAME -> faker.address()
        .cityName()

      InputRandomType.TEXT_COUNTRY_NAME -> faker.address()
        .country()

      InputRandomType.TEXT_COLOR -> faker.color()
        .name()
