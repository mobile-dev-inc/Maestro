...
object AndroidAppFiles {
    ...
    internal fun pull(dadb: DadbConnection, packageName: String, zipOutFile: File) {
        zipOutFile.delete()
    ...
    internal fun push(dadb: DadbConnection, packageName: String, appFilesZip: File) {
    ...
    private fun pullAppFile(dadb: DadbConnection, packageName: String, localPath: Path, remotePath: String) {
        dadb.open("exec:run-as $packageName cat $remotePath").use { stream ->
            Files.copy(stream.source.inputStream(), localPath)
        }
    }

    private fun listRemoteFiles(dadb: DadbConnection, packageName: String, options: String): List<String> {
    ...
    private fun shell(dadb: DadbConnection, command: String): String {
